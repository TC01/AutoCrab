...
from AutoCrab.AutoCrab3 import aliases
from AutoCrab.AutoCrab3 import crabutil
from AutoCrab.AutoCrab3 import commands
...
def doAutoCrab(command, recursive=False):
	...
	for path, dirs, files in os.walk(os.getcwd()):
		...
		if getCommandType(command) == aliases.CFG_COMMAND:
			for file in files:
				if not crabutil.isCrabConfig(os.path.join(path, file)):
					continue
